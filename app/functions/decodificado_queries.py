"""
Consultas sobre colecciones {IMEI}_OFICIAL_{AÑO} (datos decodificados / procesados).
...
from app.database.mongodb import bd_gene_oficial, collection
from app.functions.device_queries import _rango_desde_datos
from app.functions.live_helpers import _imeis_fallback_mongo
...
async def _ultimo_oficial_for_imei(imei: str) -> dict:
    """Último registro en {IMEI}_OFICIAL_{año} revisando los últimos años."""
    imei = (imei or "").strip()
    if not imei:
        return {
            "ultimo": None,
    ...
    if best is None:
        return {
            "ultimo": None,
            "coleccion": bd_gene_oficial(imei, y0),
            "anio": y0,
...
async def buscar_live_oficial(imei: str) -> dict:
    """Compatibilidad: un solo IMEI completo."""
    return await _ultimo_oficial_for_imei(imei)
...
async def buscar_live_oficial_parcial(datos: dict, tipo: str) -> dict:
    ...
    (índice por tipo) y en dispositivos; varias coincidencias devuelven detalle.
    """
    q = (datos.get("imei") or "").strip()
    ref = server_now().isoformat()
    if not q:
        return await _ultimo_oficial_for_imei("")

    if len(q) < redis_service.MIN_PARCIAL_LEN_IMEI:
        return await _ultimo_oficial_for_imei(q)

    candidates = await redis_service.buscar_imeis_parciales(tipo, q, limit=_MAX_CANDIDATOS_LIVE)
    if not candidates:
        candidates = await _imeis_fallback_mongo(tipo, q, _MAX_CANDIDATOS_LIVE)
    if not candidates:
        return await _ultimo_oficial_for_imei(q)

    detalle: list[dict] = []
    for im in candidates:
        r = await _ultimo_oficial_for_imei(im)
...
async def buscar_decodificado_imei_rango(datos: dict) -> dict:
    """
    Por IMEI y rango de fechas (DD-MM-YYYY_HH-MM-SS). Sin fechas: últimas 12 h.
    Consulta {IMEI}_OFICIAL_{año} por cada año civil que cruce el rango.
    """
