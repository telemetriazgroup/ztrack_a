...
Colecciones: TUNEL_{imei}_MM_YYYY, TUNEL_dispositivos_MM_YYYY, TUNEL_control_MM_YYYY.
"""
...
async def listar_dispositivos_tunel(
...
async def reporte_dispositivos_tunel(
    mes: Optional[int] = None,
    anio: Optional[int] = None,
) -> dict:
    m, y = _resolve_mes_anio(mes, anio)
...
async def reporte_dispositivos_tunel_global(
    mes_desde: int,
    anio_desde: int,
    mes_hasta: int,
    anio_hasta: int,
...
async def insertar_comando(datos: dict) -> dict:
    """Inserta comando en TUNEL_control_MM_YYYY."""
...
async def buscar_comandos_tunel(
